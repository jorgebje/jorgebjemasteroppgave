...
class RadixSortPara{
	...
	public void sort(int[] array1, int[] array2, int bits){
		...
		//find the global max
		int global_max_bit = workers[0].local_max_bit;
		for(int i = 1; i < thread_count; i++){
			if(workers[i].local_max_bit > global_max_bit){
				global_max_bit = workers[i].local_max_bit;
			}
		}
	...
	class Worker implements Runnable{
		...
		public void run(){
			while(open){
				...
				switch(task){
					...
					case 4: createFinalBuckets(); break;
		...
		private void getMSBPos(){
			int max = array_source[from];
			for(int i = from+1; i < to; i++){
				if(array_source[i] > max) max = array_source[i];
			}
			local_max_bit = 0;
			while(max > 0){
				max = max >>> 1;
